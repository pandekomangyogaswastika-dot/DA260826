      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-foreground/60 mt-0.5">
            ...
            platformnya. <b>Marketing hanya melihat</b> — input & jurnal dikerjakan Finance.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <ExportCsvButton
            ...
            rows={rows.map((r) => ({
              ...
              status: r.math_verified ? 'seimbang' : `selisih ${r.variance || 0}`,
              sudah_dijurnal: r.journal_id ? 'ya' : 'belum',
            }))}
          />
                    ...
                    <td className="py-2 px-3">
                      {r.math_verified ? (
                        ...
                        <Badge variant="outline" className="text-[10px] text-red-600">
                          <AlertTriangle className="w-3 h-3 mr-1" /> selisih {rp(r.variance || 0)}
                        </Badge>
                      )}
                      {r.journal_id ? (
                        <span className="ml-1 text-[10px] text-foreground/50">sudah dijurnal</span>
                      ) : null}
                    </td>
                  </tr>
                ))}
